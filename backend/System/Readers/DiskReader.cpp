...
QVariantList DiskReader::getHomeFoldersUsage() {
    ...
    QFileInfoList list = homeDir.entryInfoList();
    for (const QFileInfo &fileInfo : list) {
        if (fileInfo.fileName().startsWith(".")) continue; 

        qint64 sizeBytes = calculateDirSize(fileInfo.absoluteFilePath());
        double sizeMb = sizeBytes / (1024.0 * 1024.0);

        QVariantMap folderData;
        folderData["name"] = fileInfo.fileName();
    ...
    for (const QStorageInfo &storage : QStorageInfo::mountedVolumes()) {
        if (storage.isValid() && storage.isReady()) {
            ...
            if (path == "/" || path == "/home" || path.startsWith("/run/media")) {
                QVariantMap pData;
                pData["path"] = path;
                pData["totalGb"] = std::round((storage.bytesTotal() / 1073741824.0) * 10.0) / 10.0;
                pData["usedGb"] = std::round(((storage.bytesTotal() - storage.bytesAvailable()) / 1073741824.0) * 10.0) / 10.0;
...
QVariantMap DiskReader::getMaintenanceInfo() {
    ...
    mainData["cacheMb"] = std::round((calculateDirSize(cachePath) / 1048576.0) * 10.0) / 10.0;
    mainData["trashMb"] = std::round((calculateDirSize(trashPath) / 1048576.0) * 10.0) / 10.0;
    mainData["logsMb"] = std::round((calculateDirSize(logsPath) / 1048576.0) * 10.0) / 10.0;
