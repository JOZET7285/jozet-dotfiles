...
install_data() {
    local DATA_DIR="$HOME/.local/share/jzt"
    local TARGET_WALLPAPERS="$DATA_DIR/wallpapers"

    info "Preparing JZT data directory..."
    mkdir -p "$TARGET_WALLPAPERS"

    if [[ -d "$TARGET_WALLPAPERS" ]] &&
       [[ -n "$(find "$TARGET_WALLPAPERS" -mindepth 1 -print -quit)" ]]; then
        ok "Wallpaper directory already contains files."
    elif [[ -n "${WALLPAPERS_DIR:-}" && -d "$WALLPAPERS_DIR" ]]; then
        info "Importing wallpapers from \$WALLPAPERS_DIR..."
        cp -rn "$WALLPAPERS_DIR/." "$TARGET_WALLPAPERS/"
    elif [[ -d "$REPO_DIR/assets/wallpapers" ]]; then
        info "Installing bundled wallpapers..."
        cp -rn "$REPO_DIR/assets/wallpapers/." "$TARGET_WALLPAPERS/"
    else
        warn "No wallpapers found to install."
    fi

    ok "JZT data directory prepared."
}
